...
def NMFk(X, verbose=True, data_path='/home/ec2-user/data/malware', nrof_proc=8):
    ...
    command = f"""mpiexec -n {nrof_proc} python factor.py"""
    ...
    for rank in range(1,p_r):
        filename = 'W_' + str(rank) + '.mat'
        W = loadmat(os.path.join(data_path, filename))['W']
        W_tot = np.vstack((W_tot, W))
    
    H_tot = loadmat(os.path.join(data_path, 'H_0.mat'))['H']
    for rank in range(1,p_c):
        filename = 'H_' + str(rank) + '.mat'
        H = loadmat(os.path.join(data_path, filename))['H']
        H_tot = np.hstack((H_tot, H))
    
    k_opt = W_tot.shape[1]
    return W_tot, H_tot, k_opt


def NMFk_old(X, verbose=True, data_path='/home/ec2-user/data/malware'):
...
class HNMFk_Classifier():
    ...
    def HNMFk(self, X, y, index_mapping=None, verbose=False):
        ...
        for cluster_idx, cluster in clusters.items():
            if len(cluster)>0:
                ...
                if cluster_uniformity_score < self.cluster_uniformity_threshold:
                    ...
                    self.HNMFk(X_new, y_new, index_mapping_new, verbose=verbose)  # if X_new.shape[0] > X_new.shape[1]:
    ...
    def predict(self, verbose=False):
        self.HNMFk(self.X, self.y, verbose=verbose)
...
def MalwareDNA_train(verbose):
    ...
    pred = classifier.predict(verbose=verbose)
    #print(pred)
    #print(change_labels)
...
def load_selected_data(selected_families=['ramnit', 'adposhel', 'emotet', 'zusy'],
                       nrof_samples_per_family=1000,
                       data_set_type='train',
                       ndim=645,
                       data_dir='/home/ec2-user/data/ember2018/',
                       load_dataframe=False,
                       load_data=False):
    
    if load_data:
        with open('/home/ec2-user/data/X.pkl','rb') as f:
            X = pickle.load(f)
        
        with open('/home/ec2-user/data/y.pkl','rb') as f:
            y = pickle.load(f)
        
        with open('/home/ec2-user/data/y_label.pkl','rb') as f:
            y_label = pickle.load(f)

        return X, y, y_label
    
    if load_dataframe:
        df = pd.read_pickle('/home/ec2-user/data/df_select.pkl')
    else:
        metadata_dataframe = ember.read_metadata('/home/ec2-user/data/ember2018/')
        df = metadata_dataframe[(metadata_dataframe['subset'] == data_set_type) & (metadata_dataframe['label'] == 1)]
        df = df.dropna()
        
        df_selected = df[df['avclass'].isin(selected_families)][['avclass']]
        
        filename = 'X_' + data_set_type + '.dat'
        datapath = os.path.join(data_dir, filename)
        
        selected_index = df_selected.index
        if data_set_type == 'train':
            nrof_samples = 800_000
        else:
            nrof_samples = 200_000
            selected_index -= 800_000
            df_selected.set_index(selected_index, inplace=True)
        ...
        df_X = pd.DataFrame(X_select)
        df_X = df_X.add_prefix('feat_')
        df_X.set_index(selected_index, inplace=True)
        df_X = pd.concat([df_X, df_selected], axis=1)
        
        #df_tmp = df_X['avclass'].value_counts().to_frame()
        #print(df_tmp)  # we see that there are at least 8,000 samples of each family
        df_sampled = df_X.groupby('avclass').apply(lambda x: x.sample(n=nrof_samples_per_family, random_state=42))
        df_shuffled = df_sampled.sample(frac=1, random_state=42).reset_index(drop=True)  # shuffle
        
        #df_tmp = df_shuffled['avclass'].value_counts().to_frame()
        #print(df_tmp)
        #print(df_shuffled)
        
        X_shuffled = df_shuffled.drop(columns='avclass').to_numpy()
        
        X_scaled, mean_values, std_values, scaler = scale_data(X_shuffled)
        
        df = pd.DataFrame(X_scaled)
        df = df.add_prefix('feat_')
        df['data_set_type'] = data_set_type
        df['avclass'] = df_shuffled['avclass']
        
        df.to_pickle('/home/ec2-user/data/df_select.pkl')
    
    X = df.drop(columns=['data_set_type', 'avclass']).to_numpy()
    y_label = list(df['avclass'].values)
    
    mapping_dict = {}
    encoder = ce.one_hot.OneHotEncoder(cols=['avclass'])
    df_trans = encoder.fit_transform(df)
    ...
    with open('/home/ec2-user/data/X.pkl','wb') as f:
        pickle.dump(X, f)
    
    with open('/home/ec2-user/data/y.pkl','wb') as f:
        pickle.dump(y, f)
    
    with open('/home/ec2-user/data/y_label.pkl','wb') as f:
        pickle.dump(y_label, f)
    
    return X, y, y_label


def main_variant(verbose=False):
    X, y, y_labels = load_selected_data(load_data=True)
    
    y_new_labels, change_dict = make_unknown_labels(y_labels, percent=5)
    #print(len(y_new_labels))
    #print(len(change_dict))

    print(f"""verbose: {verbose}""")
    classifier = HNMFk_Classifier(X=X, y=y_new_labels, cluster_uniformity_threshold=1.0)
    pred = classifier.predict(verbose=verbose)
    collect_stats(truth=y_labels, pred=pred, change_dict=change_dict)


def inference_variant(sig_arch_filepath='/home/ec2-user/data/malware/signature_archive.pkl'):
    with open(sig_arch_filepath, 'rb') as f:
        signature_archive = pickle.load(f)  # signature_archive
...
#        print(label)
    ...
    nrof_error = 0
    nrof_signatures = len(corresponding_labels)
    for row_idx, true_label in tqdm(change_dict.items()):
        x_sample = X[row_idx, :]
        coefficients, _ = nnls(H_selected, x_sample)  # Solve NNLS problem: W_selected * coefficients ≈ x_row
        ...
        if len(pred_list) == 1:
            pred_label = pred_list[0]
            if pred_label != true_label:
                nrof_error += 1
            else:
                nrof_correct += 1
        elif len(pred_list) > 1:
            print("More than one predicted label.")
